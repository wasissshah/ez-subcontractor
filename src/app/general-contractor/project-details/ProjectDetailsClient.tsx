// app/general-contractor/project-details/[id]/ProjectDetailsClient.tsx
'use client';
...
export default function ProjectDetailsClient({ projectId }: { projectId: string }) {
    ...
    const getStatusConfig = (status: string | undefined) => {
        if (!status) return { bg: '#8E8E9310', color: '#8E8E93', label: 'Unknown' };
        ...
        return { bg: '#8E8E9310', color: '#8E8E93', label: status.charAt(0).toUpperCase() + status.slice(1) };
    };
                                ...
                                <div className="custom-card mb-4">
                                    <div className="mb-4 d-flex align-items-center justify-content-between gap-2 flex-wrap">
                                        ...
                                        <div className="d-flex align-items-center gap-2">
                                            ...
                                            <span style={{ backgroundColor: bg, color: color }} className="btn pt-2 pb-2 ps-3 pe-3">
                        {label}
                      </span>
                                        </div>
                                    </div>
                                    ...
                                    {images.length > 0 && (
                                        <>
                                            ...
                                            <div className="gallery-images mb-3">
                                                {images.map((img) => (
                                                    <div key={img.id} onClick={openAttachmentsModal} style={{ cursor: 'pointer' }}>
                                                        <Image
                                                            src={`${process.env.NEXT_PUBLIC_API_BASE_URL}${img.file.replace(/^\/+/, '')}`}
                                                            width={112}
                                                            height={117}
                                            ...
                                            <div className="pdf-wrapper mb-3">
                                                {pdfs.map((file, i) => (
                                                    ...
                                                    >
                                                        ...
                                                        {file.description && <p className="mb-0">{file.description}</p>}
                                                    </div>
                                                ))}
                                            </div>
                                        </>
                                            ...
                                            {pdfs.map((file) => (
                                                <div key={file.id} className="list-group-item d-flex align-items-center gap-3">
                                                    ...
                                                    <div className="flex-grow-1">
                                                        ...
                                                        {file.description && <div className="mt-1 small">{file.description}</div>}
                                                    </div>
                                                    <Link
                                                        href={`${process.env.NEXT_PUBLIC_API_BASE_URL}${file.file.replace(/^\/+/, '')}`}
                                                        target="_blank"
